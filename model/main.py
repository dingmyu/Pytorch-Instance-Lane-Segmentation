...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target_ins, n_objects) in enumerate(train_loader):
        ...
        x_sem, x_ins, x_cls= model(input_var)
        ...
        loss = loss_seg + loss_ins + loss_cls
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Lr {lr.val:.5f} ({lr.avg:.5f})\t'.format(
                   ...
                   data_time=data_time, loss_seg=losses_seg, loss_ins=losses_ins, loss_cls=losses_cls, lr=lrs))
